...
<x-content-card title="Daftar Pasien yang Sudah Masuk Lab">
    <x-slot name="body">
        <div class="row">
            <div class="col-lg-12">
                ...
                <table class="table table-sm table-striped table-hover" id="tbl_data">
                    ...
                    <tbody>
                        @foreach ($antrian as $q)
                            <tr>
                                <td><button class="btn btn-sm btn-warning btn-block" onclick="goBatalMasuk({{ $q->id_pasien_lab }});">Balik</button></td>
                                <td><button class="btn btn-sm btn-success btn-block" onclick="goFind({{ $q->id_pasien_lab }});">Detail</button></td>
                                <td><button class="btn btn-sm btn-primary btn-block" onclick="goSelesaiPeriksaLab({{ $q->id_pasien_lab }});">Selesai</button></td>
...
<script type="text/javascript">
    ...
    function goBatalMasuk(id_pasien_lab) {
        if(window.confirm("Apakah anda yakin bahwa pasien ini batal masuk lab dan balik ke dalam antrian?")) {
            document.location.href = "{{ url('/antrian_lab/pasien_batal_masuk') }}?id_pasien_lab=" + id_pasien_lab;
        }
    }

    function goSelesaiPeriksaLab(id_pasien_lab) {
        if(window.confirm("Apakah anda yakin bahwa pasien ini sudah selesai melakukan pemeriksaan lab?")) {
            document.location.href = "{{ url('/antrian_lab/pasien_selesai_periksa_lab') }}?id_pasien_lab=" + id_pasien_lab;
        }
    }
</script>
